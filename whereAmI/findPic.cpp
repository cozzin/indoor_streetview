...
void modifyDirectionMap(PTS_INFO* io_dstMap, char * in_directionMapFileName,

	int in_pts_num); // 관계표 수정
...
int main(){
	...
	while ((num_pic < indexNum) || (indexNum < 0) || (choice < 0)){
		...
		if ((num_pic < indexNum) || (indexNum < -2)){
			...
			cin >> startIdx;
			cin >> endIdx;
			shortestPath(endIdx, startIdx, g);
			}
		else {
...
void shortestPath(int startIdx, int endIdx, pGraph g){
	...
	cout << endIdx << "부터 " << startIdx << "까지의 거리를 계산하겠습니다. " << endl;
	pGraph h = newGraph(V);
	pq p = newPQ(V);
	pq S = newPQ(V);
	double distance = 0.;
	...
	distance = dijkstra(g, p, S, startIdx, endIdx);
	cout << endIdx << "부터 " << startIdx << "까지의 거리는 " << distance << " 입니다" << endl;
	system("pause");
}