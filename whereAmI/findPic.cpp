...
typedef struct pts_info{
	...
	int floor; // 층 정보
	char tagging[MAX_TEXT]; // 장소 정보 태깅
...
void makeDirectionMap(PTS_INFO* io_dstMap, int &out_num_pic, char * directionMapFileName); // 관계표 만들기
...
void readTextDirectionMap(char * in_directionMapFileName, PTS_INFO* &io_dstMap); // 관계표 txt 읽어오기
...
int main(){
	...
	makeDirectionMap(dirMap, num_pic, directionMapFileName); // 관계표 만들기
	...
	while ((num_pic < indexNum) || (indexNum < 0)){
		...
		cout << "관계표 수정하려면 -1, 인덱스 번호로 이동하려면 0 ~ " << (num_pic - 1) << " 입력" << endl;
