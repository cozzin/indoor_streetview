...
void hough_transform(Mat& im, Mat& orig, double* skew)
{
	...
	/* 원래 코드
	Point maxLoc;
	minMaxLoc(acc, 0, 0, 0, &maxLoc);	// 가장 뚜렷한 직선값
	...
	else if (rho * theta > 0){	// 수직 (y축)에 가까운 선 검출
		//convert normal form back to slope intercept form
		//y = mx + b
		double m = sin(theta) / cos(theta);	// 세로선
		double b = rho / sin(theta) + im.rows / 2. - m*im.cols / 2.;
		Point2d p1 = Point2d(0, b);
		Point2d p2 = Point2d(im.cols, im.cols*m + b);
		line(orig, p1, p2, Scalar(0, 0, 255), 1);
		double skewangle;
		skewangle = p1.x - p2.x>0 ? -(atan2(p1.x - p2.x, p1.y - p2.y)*180. / CV_PI) : -(atan2(p1.x - p2.x, p1.y - p2.y)*180. / CV_PI);	// 세로선
		*skew = skewangle;
		cout << "skew angle " << skewangle << endl;
	}
	else // 수평 (x축)에 가까운 선 검출
	{ 
		...
		//convert normal form back to slope intercept form
		//y = mx + b
		double m = -cos(theta) / sin(theta);	// 가로선
		double b = rho / sin(theta) + im.rows / 2. - m*im.cols / 2.;
		Point2d p1 = Point2d(0, b);
		Point2d p2 = Point2d(im.cols, im.cols*m + b);
		line(orig, p1, p2, Scalar(0, 0, 255), 1);
...
{
	string src = "OH\\idx3_camera3.jpg";
	Mat im = imread(src);
	Mat gray;
